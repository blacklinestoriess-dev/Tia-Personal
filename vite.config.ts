...
function tiaApiPlugin(): Plugin {
  return {
    name: 'vite-plugin-tia-api',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (req.url && req.url.startsWith('/api/profile')) {
...
export default defineConfig(() => {
  return {
    plugins: [react(), tailwindcss(), aistudioMediaPlugin(), tiaApiPlugin()],
    resolve: {
      alias: {
